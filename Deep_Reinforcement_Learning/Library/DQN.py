# -*- coding: utf-8 -*-
"""
Created on Fri Aug  3 13:47:59 2018
...
class DQN:
    def __init__(self, n_inputs, n_outputs, hidden_layer_sizes, gamma,
        ...
        # Create Fully Connected Layers Graph Abstraction
        self.layers = []
        input_size = self.n_inputs
        for layer_size in hidden_layer_sizes:
            layer = FCL.FullyConnectedLayer(input_size, layer_size)
            self.layers.append(layer)
            input_size = layer_size
        
        # Append Final Fully Connected layer
        layer = FCL.FullyConnectedLayer(input_size, self.n_outputs,activation_fun = None) # final layer does not take an activation function
        self.layers.append(layer)
        ...
        # inputs and targets
        xdim = [None] + list(n_inputs)
        self.X = tf.placeholder(tf.float32, shape= xdim, name='X') # None = it can vary
