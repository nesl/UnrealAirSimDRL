...
class DQN:
    ...
    def sample_action(self,obs4,eps):
        ...
        obs4 = obs4.reshape((1,np.size(obs4)))
        actionNum = self.pget_action_argnum(obs4,eps)
    ...
    def state_stack(self, obs4, obs): 
        # will only occur if obs3 is empty..ie after a reset.
        if len(obs4) == 0:
    ...
    def play_game(self, TrainingModel, env, rounds = 1, copy_period = 40):
        ...
        for i in range(rounds):
            ...
            obs4 = []
            obs = env.reset()
            obs4 = self.state_stack(obs4,obs) # fills obs4 with same info 4 times
            ...
            while not done and it < 350:
                local_round += 1
                action, actionNum = self.sample_action(obs4, ep)
                
                # Update stacked state
                prev_obs4 = obs4
                obs, reward, done = env.step(action)
                obs4 = self.state_stack(obs4,obs) # pop old and append new state info
                ...
                self.add_experience(prev_obs4, actionNum, reward, obs4, done)
...
def main():
    ...
    minimax_z_thresh = (.1, 3.5) # in absolute z-coords
    
    env = environment(setpoint,minimax_z_thresh,max_xy_drift)
    FRAMES = 4
    frame_inputs = 8 # roll, pitch, Vx, Vy, Vz, Pos x, y, z 
    n_inputs = FRAMES*frame_inputs
