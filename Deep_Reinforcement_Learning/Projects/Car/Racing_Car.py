...
def train_racing_car(primaryNetwork, targetNetwork, 
    ...
    for episode in range(NUM_EPISODES):
        ...
        obs_img = PIL.Image.fromarray(np.uint8(obs*255))
        obs = np.array(obs_img.resize((IMG_HEIGHT, IMG_WIDTH), PIL.Image.ANTIALIAS), dtype = np.float32)
        ...
        while not DONE_FLAG and episode_iterator < EPISODE_ITERATIONS_MAX:
            ...
            print("Obs4 Shape: ", obs4.shape, "Obs Shape: ", obs.shape)
...
def main():
    ...
    UREnv = CarUnrealEnvironment(mode = "both-rgba") # Returns both the image state (RGB) and the inertial state
