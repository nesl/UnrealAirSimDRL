...
class FullyConnectedResNetBlock:
    ...
    def __init__(self, in_dim, hl_dims, bias = False, activation_fun = tf.nn.relu, batch_normalization = True):
        self.in_dim = in_dim
        ...
        for i in range(len(hl_dims)):
            if i == 0:  # Give Relu NN activation function
                self.fc_layers.append(FullyConnectedLayer(
                    idim, hl_dims[i], bias=self.bias, activation_fun=activation_fun))
