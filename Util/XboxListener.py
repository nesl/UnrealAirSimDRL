...
# Globals
keys = ["LA","LAV","LB", "LBIP", "Time"]
LAST_AXIS = 0
LAST_AXIS_VALUE = 0
LAST_BUTTON = 0
LAST_BUTTON_IS_PRESSED = 0
lock = threading.Lock()
xboxq = XboxQ()
...
class XINPUT_GAMEPAD(ctypes.Structure):
    _fields_ = [
        ...
        ('lt', ctypes.c_ubyte),  # bLeftTrigger
        ('rt', ctypes.c_ubyte),  # bLeftTrigger
...
def sample_first_joystick(dt):
    ...
    """
    os.chdir(os.getcwd()) # File Desination
    xbox_in = open("Xboxin.csv", "w+")
    xbox_in.write("Button/Axis" + ',' + "Value"+ ',' + "Time" + "\n")
    ...
    def on_button(button, pressed):
        now = time.time() - start
        if (button == 5): #press menu button to exit
            xbox_in.close()
            sys.exit(0)
        #print('button', button, pressed, now)
        global LAST_BUTTON, LAST_BUTTON_IS_PRESSED, LAST_AXIS, LAST_AXIS_VALUE, xboxq
        LAST_BUTTON = button
        LAST_BUTTON_IS_PRESSED = pressed
        xboxInput = {keys[0]: LAST_AXIS, keys[1]: LAST_AXIS_VALUE, 
                     keys[2]: LAST_BUTTON, keys[3]: LAST_BUTTON_IS_PRESSED, keys[4]: now}
        xboxq.put(xboxInput)
